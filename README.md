...
<img src="./public/sonic.png" alt="Sonic" width="720" />
...
<p>AI-TTS generation and voice cloning made with Next.js 16, React 19, and Chatterbox TTS.</p>
...
<p>
  ...
  <a href="https://railway.com/"><img src="https://img.shields.io/badge/Railway-0B0D0E?style=for-the-badge&logo=railway&logoColor=white" alt="Railway" /></a>&nbsp;
...
- [Cloudflare R2](https://www.cloudflare.com/) bucket
...
```bash
git clone https://github.com/all-black-493/sonic.git
cd sonic
npm install
```
...
In your [Polar](https://polar.sh) dashboard, create two **meters** under **Meters**:
...
1. Click **Add metered price** and select the **Text-to-Speech Characters** meter
   - Set the **Amount per unit** (price per character, e.g. `$0.005`)
   - Optionally set a **Cap amount** (e.g. `$100`)

2. Click **Add metered price** again and select the **Voice Creation** meter
   - Set the **Amount per unit** (price per voice generation, e.g. `$0.30`)
   - Optionally set a **Cap amount** (e.g. `$100`)
...
Sonic is designed to be self-hosted. You'll need:
...
│   ├── api/                    # Audio proxy routes + oRPC handler (contract-first development, not schema-first)
...
├── orpc/                       # oRPC routers, client, server helpers
